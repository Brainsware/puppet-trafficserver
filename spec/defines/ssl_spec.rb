...
describe 'trafficserver::config::ssl', :type => :define do
	...
	context 'README test: single' do
		 ...
		 it { should contain_concat__fragment('/etc/trafficserver/ssl_multicert.config_README example: single' ) \
	...
	context 'README test: multiple' do
		 ...
		 it { should contain_concat__fragment('/etc/trafficserver/ssl_multicert.config_README example: multiple' ) \
				.with_content(/ssl_cert_name=.+mail.+crt ssl_key_name=.+mail.+key \n/)
